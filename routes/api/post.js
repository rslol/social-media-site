...
router.get('/', (req, res) => {
    Post.find()
        // How to order posts from new to old 
        .sort({ date: -1 })
        ...
        .then(profile => {
            Post.findById(req.params.id)
                .then(post => {
                    // Check if user id is already in likes array
                   if(post.likes.filter(like => like.user.toString() === req.user.id).length == 0) {
                       return res.status(400).send({ notliked: 'User has not liked post'});
                   }
                   // Get remove index
    ...
    Post.findById(req.params.id)
        ...
        //  Check to see if comment exist 
            if(post.comment.filter(comment => comment._id.toString() === req.params.comment_id).length === 0) {
                return res.json(404).json({ commentnotexists: 'Comment not found' });
            }
            // Get remove index
